...
func _input(event):
	if event is InputEventKey and event.pressed:
		match event.keycode:
			KEY_A, KEY_Q:
				update_diamond(diamond_1, true)
			KEY_S, KEY_W:
				update_diamond(diamond_2, true)
			KEY_D, KEY_E:
				update_diamond(diamond_3, true)
			KEY_F, KEY_R:
				update_diamond(diamond_4, true)
			KEY_J, KEY_U:
				update_diamond(diamond_5, true)
			KEY_K, KEY_I:
				update_diamond(diamond_6, true)
			KEY_L, KEY_O:
				update_diamond(diamond_7, true)
			KEY_SEMICOLON, KEY_P:
				update_diamond(diamond_8, true)

	elif event is InputEventKey and not event.pressed:
		match event.keycode:
			KEY_A, KEY_Q:
				update_diamond(diamond_1, false)
			KEY_S, KEY_W:
				update_diamond(diamond_2, false)
			KEY_D, KEY_E:
				update_diamond(diamond_3, false)
			KEY_F, KEY_R:
				update_diamond(diamond_4, false)
			KEY_J, KEY_U:
				update_diamond(diamond_5, false)
			KEY_K, KEY_I:
				update_diamond(diamond_6, false)
			KEY_L, KEY_O:
				update_diamond(diamond_7, false)
			KEY_SEMICOLON, KEY_P:
				update_diamond(diamond_8, false)

func update_diamond(diamond, is_pressed):
	if is_pressed:
